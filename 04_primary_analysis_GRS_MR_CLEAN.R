...
################################################################
cytosnp_linkages <- fread(" ## ADD LINKAGE FILE ##", header=TRUE)
gsa_linkages <- fread(" ## ADD LINKAGE FILE ##", header=TRUE)
ugli2_linkages <-fread(" ## ADD LINKAGE FILE ##", header=TRUE)
...
PC_cyto <- fread("## ADD PCS FILE ##", header=TRUE)
PC_gsa <- fread(" ADD PCS FILE ##")
PC_ugli2 <- fread("## ADD PCS FILE ##", header=TRUE)
...
cytosnp_duplicates_and_1stdegree_UGLI <- fread("## ADD FILE ##", header=FALSE)
cytosnp_duplicates_and_1stdegree_UGLI2 <- fread("## ADD FILE ##", header=FALSE)
...
## Checks - UGLI file all distinct, but UGLI2 file has some individuals not distinct in list
...
non_euro_gsa <- fread("## Upload List ##",header=TRUE)
...
#check n non-euro in genetics data (not merged file) 
...
GSA_duplicates_and_1stdegree_UGLI2 <- fread(" ## ADD FILE HERE ##", header=FALSE)
...
###########################################################
ugli2_noneuro <- fread("## ADD FILE HERE ##", header=FALSE)
ugli2_noneuro <- rename(ugli2_noneuro, FID=V1, IID=V2) 
ugli2_geneticoutliers <- fread("## ADD FILE HERE ##", header=TRUE)
...
PCs_on_merged_data <- fread(" ## ADD FILE HERE ##", header=TRUE)
...
lifelines_combined_1stremoved <- lifelines_combined[! lifelines_combined$IID %in% gsa_kingexclusions_177_phenoavail$IID,]
lifelines_combined_1stremoved <- lifelines_combined[! lifelines_combined$IID %in% ugli2_kingexclusions_177_phenoavail$IID,]
...
lifelines_combined_2ndremoved <- lifelines_combined[! lifelines_combined$IID %in% gsa_kingexclusions_088_phenoavail$IID,]
lifelines_combined_2ndremoved <- lifelines_combined[! lifelines_combined$IID %in% ugli2_kingexclusions_088_phenoavail$IID,]
...
lifelines_combined_3rdremoved <- lifelines_combined[! lifelines_combined$IID %in% gsa_kingexclusions_044_phenoavail$IID,]
lifelines_combined_3rdremoved <- lifelines_combined[! lifelines_combined$IID %in% ugli2_kingexclusions_044_phenoavail$IID,]
...
###########################################
## Note: adjusted analysis (sex and age) slightly lower N (few without gender data)
####################################
######### Adjusted (grammar) #######
####################################
...
## Checks - why lower when include age and gender - few without gender data
...
#write.table(GRS_outcomes_continuous_1stdeg_rem, file="LL_GRS_outcomes_continuous_1stdeg_removed.txt", row.names=TRUE, col.names=TRUE, quote=FALSE, sep="\t")
#write.table(GRS_outcomes_binary_1stdeg_rem, file="LL_GRS_outcomes_binary_1stdeg_removed.txt", row.names=TRUE, col.names=TRUE, quote=FALSE, sep="\t")
...
#write.table(GRS_outcomes_continuous_2nddeg_rem, file="LL_GRS_outcomes_continuous_2nddeg_removed.txt", row.names=TRUE, col.names=TRUE, quote=FALSE, sep="\t")
#write.table(GRS_outcomes_binary_2nddeg_rem, file="LL_GRS_outcomes_binary_2nddeg_removed.txt", row.names=TRUE, col.names=TRUE, quote=FALSE, sep="\t")
...
#write.table(GRS_outcomes_continuous_3rddeg_rem, file="LL_GRS_outcomes_continuous_3rddeg_removed.txt", row.names=TRUE, col.names=TRUE, quote=FALSE, sep="\t")
#write.table(GRS_outcomes_binary_3rddeg_rem, file="LL_GRS_outcomes_binary_3rddeg_removed.txt", row.names=TRUE, col.names=TRUE, quote=FALSE, sep="\t")
...
#capture.output(tsls_panasneg_saidcis_1stremoved, file="LL_tsls_panasneg_saidcis_1stremoved.txt")
...
#capture.output(tsls_minianx_saidcis_1stremoved, file="LL_tsls_minianx_saidcis_1stremoved.txt")
...
#capture.output(tsls_panasneg_saidcis_2ndremoved, file="LL_tsls_panasneg_saidcis_2ndremoved.txt")
...
#capture.output(tsls_minianx_saidcis_2ndremoved, file="LL_tsls_minianx_saidcis_2ndremoved.txt")
...
#capture.output(tsls_panasneg_saidcis_3rdremoved, file="LL_tsls_panasneg_saidcis_3rdremoved.txt")
...
#capture.output(tsls_minianx_saidcis_3rdremoved, file="LL_tsls_minianx_saidcis_3rdremoved.txt")
